...
const Vouchers = ({ data, rows, user, hideVoucher }) => {
  ...
  const redeemVoucher = () => {
    setLoading(true)
    let url =
      "https://us-central1-georgia-on-my-dime.cloudfunctions.net/redeemVoucher"
    Axios.post(url, {
      uid: user.uid,
            ...
            return (
              <Card key={voucher.id} link>
                ...
                <Card.Content>
                  ...
                  <Card.Description style={{ minHeight: "40px" }}>
                    {voucher.description}
                  </Card.Description>
                  <div style={{ textAlign: "center", marginTop: "5px" }}>
                    <Responsive {...Responsive.onlyMobile}>
                      <Modal
                        ...
                        trigger={
                          <Button
                            onClick={handleOpen.bind(this, voucher)}
                            primary
                          >
                            Redeem
                          </Button>
                        ...
                        {redeeming ? (
                            ...
                            <Modal.Content>
                              ...
                              >
                                {checkExpiration(
                                  ...
                                  parseInt(activeVourcher.duration)
                                ) === "Expired" ? (
                                  <p style={{ color: "red" }}>
                                    EXPIRED: Redeem for{" "}
                                    {formatCurrency(activeVourcher.price)}
                                  </p>
                                ) : (
                                  <p>{activeVourcher.description}</p>
                                )}
                                <Barcode
                                  value={activeVourcher.couponCode}
                                ></Barcode>
                              </Modal.Description>
                            </Modal.Content>
                            <Modal.Actions>
                          ...
                          <>
                            <Modal.Header style={{ textAlign: "center" }}>
                              <span style={{ color: "red" }}>WAIT! </span>Do not
                              redeem this voucher until check out.
                            </Modal.Header>
                            <Modal.Content>
                              <Modal.Description>
                                <p>
                                  Wait until you are ready to pay before
                                  redeeming this voucher.{" "}
                                  <span style={{ fontWeight: "bold" }}>
                                    This voucher will disappear automatically
                                    after use.
                                  </span>
                                </p>
                                <p>
                                  When you are ready, press redeem and show the
                                  screen to your waiter/waitress.{" "}
                                </p>
                              </Modal.Description>
                            </Modal.Content>
                            <Modal.Actions>
                    ...
                    <Responsive minWidth={768}>
                      <p style={{ fontSize: "12px" }}>
                        <i>
                          Must redeem in store. Try again on a mobile device
                        </i>
                      </p>
                    </Responsive>
                  </div>
                      ...
                      <Modal.Description>
                        <h2>
                          ...
                          {voucher.purchaseDate
                            ? checkExpiration(
                                voucher.purchaseDate.seconds,
                                parseInt(voucher.duration)
                              )
                            : null}
                        </h2>
                        <h4>
                          Purchase Price: {formatCurrency(
                            voucher.price
                          )} <br /> Redeem For: {voucher.description}
                        </h4>
                        <div>
