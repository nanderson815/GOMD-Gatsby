...
const HappyHourMap = ({ happyHours, hovered }) => {
  const center = { lat: 33.78454, lng: -84.388 }
  const zoom = 11
  ...
  useEffect(() => {
    if (happyHours.length > 0) {
      const points = happyHours.map(m => (
        ...
        >
          {visible === m.id && (
            <InfoWindow style={{ padding: '0px' }} onCloseClick={() => onClickHandler(m.id)}>
              <Card style={{ width: '200px' }}>
                <Img style={{ height: '100px' }} alt={m.name} fluid={m.mainImg.fluid} />
                <Card.Content>
                  <Card.Header>
                    <Link style={{ color: 'black' }} to={`/atlanta-happy-hour/${m.slug}`}>
                      {' '}
                      {m.name}
                    </Link>
                  </Card.Header>
  ...
  return (
    ...
    <GoogleMap defaultZoom={zoom} defaultCenter={center} options={defaultMapOptions} onClick={clearVisible}>
      <MarkerClusterer
        averageCenter
        enableRetinaIcons
        gridSize={25}
