...
const HHFinderCardGroup = ({
  ...
  useEffect(() => {
    let cards = happyHours.map((deal, index) => {
      ...
      day === "All"
        ? (descriptionString = "")
        : (descriptionString = deal[descField][descField])
      let trimmedString =
        descriptionString.length > 200
          ? descriptionString.substring(0, 200 - 3) + "..."
          : descriptionString
      ...
      return (
        ...
        >
          <Link to={`/atlanta-happy-hour/${deal.slug}`}>
            <Img
              style={{ height: "150px" }}
              alt={deal.name + " Happy Hour atlanta"}
          ...
          <Card.Content>
            <Card.Header style={{ marginBottom: "5px" }}>
              ...
              <Link
                style={{ color: "black" }}
                to={`/atlanta-happy-hour/${deal.slug}`}
              >
                {deal.name}
              </Link>
            </Card.Header>
  ...
  return (
    <Card.Group
      itemsPerRow={rows}
      style={{ marginTop: offset ? offset : "20px", paddingBottom: "55px" }}
    >
      {cards}
    </Card.Group>
  )
