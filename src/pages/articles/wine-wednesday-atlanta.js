...
const WineWednesdayAtlanta = props => {
  const { data } = props
  const wineData = data.allContentfulHappyHour.edges.map(item => item.node)
  const [center, setCenter] = React.useState('')

  const wineWednesdayHappyHours = wineData.map(deal => (
    <div data-lat={deal.location.lat} data-lon={deal.location.lon} id={deal.id} key={deal.id}>
      <Img
        alt={`${deal.name} wine wednesday atlanta`}
        title={`${deal.name} wine wednesday atlanta`}
  ...
  return (
    <div>
      <SEO
        ...
        description="Here are the Best Wine Wednesday deals in Atlanta. You've made it halfway through the week - time to reward yourself!"
      />
      <Header />
        ...
        <Grid>
          <Grid.Column computer={10} tablet={10} mobile={16}>
            <p style={{ fontWeight: '400' }}>
              There’s no reason to whine when you still have a couple more days of the work week – because guess
              what...It’s Wine Wednesday! Whether you’re looking for a fun night out with the gals or just need a little
              solo pick-me-up, we’ve got you covered on Wednesdays. Keep reading till the end – one of these spots
              offers Bottomless Wine on Wednesdays. Some might think that’s aggressive, but honestly, we like their
              style!{' '}
            </p>
            <Divider />
